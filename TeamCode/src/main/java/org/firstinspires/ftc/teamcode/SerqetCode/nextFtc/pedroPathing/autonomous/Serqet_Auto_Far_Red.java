...
@Autonomous(name = "Serqet Auto Far Red", group = "Examples")
public class Serqet_Auto_Far_Red extends OpMode {
    ...
    private final Pose startPose = new Pose(88.2, 8, Math.toRadians(90)); // Start Pose of our robot.
    private final Pose scorePose = new Pose(90.4, 13.1/*11.1*/, Math.toRadians(60)); // Scoring Pose of our robot. It is facing the goal at a 135 degree angle.
    private final Pose pickup1Pose = new Pose(130.4, 35.2, Math.toRadians(0)); // Highest (First Set) of Artifacts from the Spike Mark.
    private final Pose pickup2Pose = new Pose(129.5,58.6, Math.toRadians(0));
    private final Pose lineup1Pose = new Pose(102.6, 34.8, Math.toRadians(0));
    private final Pose lineup2Pose = new Pose(102.1,59.3,Math.toRadians(0));
    ...
    public void buildPaths() {
        ...
        lineup1Path.setLinearHeadingInterpolation(scorePose.getHeading(), lineup1Pose.getHeading(), .8);

        pickup1Path = new Path(new BezierLine(lineup1Pose, pickup1Pose)) ;
        pickup1Path.setLinearHeadingInterpolation(lineup1Pose.getHeading(), pickup1Pose.getHeading(), .8);

        score1Path = new Path(new BezierLine(pickup1Pose, scorePose));
        score1Path.setLinearHeadingInterpolation(pickup1Pose.getHeading(), scorePose.getHeading(), .8);

        lineup2Path = new Path(new BezierLine(scorePose, lineup2Pose ));
        lineup2Path.setLinearHeadingInterpolation(scorePose.getHeading(), lineup2Pose.getHeading(), .8);

        pickup2Path = new Path(new BezierLine(lineup2Pose, pickup2Pose));
        pickup2Path.setLinearHeadingInterpolation(lineup2Pose.getHeading(), pickup2Pose.getHeading(), .8);

        score2Path = new Path(new BezierLine(pickup2Pose, scorePose));
        score2Path.setLinearHeadingInterpolation(pickup2Pose.getHeading(), scorePose.getHeading(), .8);

        emptyPath = new Path(new BezierLine(scorePose, empty));
        emptyPath.setLinearHeadingInterpolation(scorePose.getHeading(), empty.getHeading(), .8);

        endPath = new Path(new BezierLine(empty, endPose));
        endPath.setLinearHeadingInterpolation(empty.getHeading(), endPose.getHeading(), .8);
