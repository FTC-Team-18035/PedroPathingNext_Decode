...
@Autonomous(name = "Close Blue 15 Artifacts", group = "Examples", preselectTeleOp = "BLUE Main TeleOp")
 public class Close_Auto_Blue_15Artifact extends OpMode {
    ...
    private final Pose scorePose = new Pose(55, 89/*11.1*/, Math.toRadians(138)); // Scoring Pose of our robot. It is facing the goal at a 135 degree angle.
    private final Pose lineup2Pose = new Pose(41.9, 60, Math.toRadians(180));
    private final Pose pickup2Pose = new Pose(21, 60, Math.toRadians(180)); // Highest (First Set) of Artifacts from the Spike Mark.
    private final Pose gatePose = new Pose(11.3, 61, Math.toRadians(140)); // Highest (First Set) of Artifacts from the Spike Mark.
    private final Pose score2Pose = new Pose(62, 82, Math.toRadians(138)); // Highest (First Set) of Artifacts from the Spike Mark.
    private final Pose lineup1Pose = new Pose(41, 83.5, Math.toRadians(180)); // Highest (First Set) of Artifacts from the Spike Mark.
    private final Pose pickup1Pose = new Pose(20, 83.5, Math.toRadians(180)); // Highest (First Set) of Artifacts from the Spike Mark.
    private final Pose lineup3Pose = new Pose(41, 35, Math.toRadians(180)); // Highest (First Set) of Artifacts from the Spike Mark.
    private final Pose pickup3Pose = new Pose(20, 35, Math.toRadians(180)); // Highest (First Set) of Artifacts from the Spike Mark.
    private final Pose score3Pose = new Pose(55, 115, Math.toRadians(148)); // Highest (First Set) of Artifacts from the Spike Mark.
    ...
    public void buildPaths() {
        ...
        readyPath = new Path(new BezierLine(scorePose, lineup2Pose));
        readyPath.setLinearHeadingInterpolation(scorePose.getHeading(), lineup2Pose.getHeading(), .8);

        lineup1Path = new Path(new BezierLine(lineup2Pose, pickup2Pose)) ;
        lineup1Path.setLinearHeadingInterpolation(lineup2Pose.getHeading(), pickup2Pose.getHeading(), .5);

        pickup1Path = new Path(new BezierLine(pickup2Pose, scorePose));
        pickup1Path.setLinearHeadingInterpolation(pickup2Pose.getHeading(), scorePose.getHeading(), .8);
