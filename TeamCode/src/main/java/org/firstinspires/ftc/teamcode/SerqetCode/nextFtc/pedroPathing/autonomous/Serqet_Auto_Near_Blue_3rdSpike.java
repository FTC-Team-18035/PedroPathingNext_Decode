...
public class Serqet_Auto_Near_Blue_3rdSpike extends OpMode {
    ...
    public void buildPaths() {
        ...
        pickup3Path = new Path(new BezierLine(pickup3Pose, score2Pose));
        pickup3Path.setLinearHeadingInterpolation(pickup3Pose.getHeading(), score2Pose.getHeading(), .8);

        score3Path = new Path(new BezierLine(score2Pose, lineup1Pose));
        score3Path.setLinearHeadingInterpolation(score2Pose.getHeading(), lineup1Pose.getHeading(), .5);
    }
    ...
    private double shootForTime(double seconds) {
        ...
        while (opmodeTimer.getElapsedTimeSeconds() < 30 && timer.seconds() < seconds + .2) {
            if(timer.seconds() > seconds) {
                ...
                intake.setPower(1); // matches BLUEMainTeleOpWORKING feeding direction
                shooter.update();
